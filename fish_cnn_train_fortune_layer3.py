...
def get_hyperparams():
    ...
    hyperparams = {
        ...
        "dr": random.choice([0.6]),
        "epoch": random.choice([20]),
        "batch": random.choice([32]),
    }
    return hyperparams
...
def inference(x, keep_prob, image_size, channel_num, class_num):
    ...
    # 画像を平坦化してベクトルにする
    dimension = out_size * out_size * conv3_out
    x_flatten = tf.reshape(pool3, [-1, dimension])

    # 全結合層
    fc = dense_layer(x_flatten, dimension, conv3_out)
    # 活性化関数
    fc = tf.nn.relu(fc)
    ...
    # モデル出力
    y = dense_layer(drop, conv3_out, class_num)
